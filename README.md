...
<p align="center"><a href="#короткий-показ">Посмотреть сценарий</a> · <a href="#попробовать">Запустить демо</a> · <a href="#архитектура">Как устроено</a></p>
...
От вопроса — к нужной переписке: поиск, выбранные письма и ответ с основаниями.
...
*11 секунд. Постановочная иллюстрация: организация, письма и ответ вымышлены; LLM не вызывается. [Посмотреть итог без анимации](docs/assets/mail-scenario-still.png).*

**Для бизнеса:** сотрудник получает переписку, на которую можно опереться в решении или ответе клиенту. Результат пилота можно оценить по времени поиска и полноте найденных договорённостей.
...
```mermaid
flowchart LR
    P["Portal / OpenWebUI<br/>Вопрос и смысловые решения"] --> S["Server / Query Engine<br/>Запросы, выборки и ограничения"]
    S --> M["Mail Connector<br/>Доступ и подготовка данных"]
```
...
<details>
<summary>Показать выполнение — 18 секунд</summary>
...
### Нужен AI для ваших процессов?

**Илья, автор проекта.** Разрабатываю AI-инструменты для бизнеса. Расскажите о задаче — обсудим решение и первый шаг.

**[Обсудить задачу в Telegram · @mia_ops →](https://t.me/mia_ops)**
